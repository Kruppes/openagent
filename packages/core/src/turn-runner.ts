  ...
  private async runAttempt(
    ...
    try {
      ...
      const stream = continueTurn && agent.retryTurn
        ? agent.retryTurn(agentUserId, input.text, input.source ?? 'web', input.attachments, turn.agentId)
        : agent.sendMessage(agentUserId, input.text, input.source ?? 'web', input.attachments, turn.agentId)
...
class TurnTranscript {
  ...
  finalize(): void {
    ...
    this.remember(
      saveChatMessage(this.db, this.sessionId, this.userId, 'assistant', this.fullResponse, metadata, this.agentId),
    )
  }
}
